...
- The local web runtime in this repository should start through `pnpm tools-dev run web --namespace <name> --daemon-port <port> --web-port <port>`. Runtime files and control endpoints are namespace-scoped, so the development worktree and baseline worktree need different namespace values as well as different daemon/web ports to run independent service pairs concurrently. Source: `AGENTS.md:40-45,82-89,91-104`
