...
- `packages/sidecar-proto`: Open Design sidecar business protocol. Owns app/mode/source constants, namespace validation, stamp descriptor/fields/flags, control message schema, status shapes, error semantics, and default product path constants.
- `packages/launcher-proto`: TypeScript launcher activation protocol. Owns launcher-compatible `launcher.json`/`runtime.json` DTOs, app constants, namespace validation, TCP loopback `endpoint` validation, and descriptor builders for the packaged activation path. It must not import `sidecar-proto` or carry source-workspace lifecycle concepts.
...
- For new shared types, choose the boundary first: web/daemon app DTOs go in `contracts`; sidecar control protocol goes in `sidecar-proto`; launcher activation descriptors go in `launcher-proto`; generic runtime code goes in `sidecar`; generic OS/process code goes in `platform`.
...
- Keep stamp fields limited to five: `app`, `mode`, `namespace`, `endpoint`, and `source`.
